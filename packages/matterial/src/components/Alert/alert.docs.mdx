...
<Alert severity="error" label="Critical Error" type="assertive">Something happened</Alert>
<Alert
  severity="warning"
  message="Danger!"
  action={
    <Button variant="contained" color="warning">Take me to safety</Button>
  }
/>
...
<Container row>
  {SEVERITIES.map(severity => <Alert key={severity} severity={severity} label={false} style={{margin:0}}>{severity}</Alert>)}
</Container>
...
<Alert icon={<Icon.Notification color="hotpink" />}>Custom</Alert>
